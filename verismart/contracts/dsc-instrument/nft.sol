contract Nft {
  ...
  function updateTokenNoOwnerOnInsertExists_r8(uint tokenId,bool true) private    {
      OwnerOfTuple memory ownerOfTuple = ownerOf[tokenId];
      if(true) {
        address o = ownerOfTuple.p;
        if(o==address(0)) {
  ...
  function updateExistsOnInsertLatestTransfer_r11(uint tokenId,address to) private    {
      if(to!=address(0)) {
        updateTokenNoOwnerOnInsertExists_r8(tokenId,bool(true));
  ...
  function updateTokenNoOwnerOnInsertOwnerOf_r8(uint tokenId,address o) private    {
      ExistsTuple memory existsTuple = exists[tokenId];
          ...
          if(s==owner.p) {
            ...
            if(true==existsTuple.b) {
              ...
              if(true) {
                address p = ownerOfTuple.p;
                if(true) {
                  updateLatestTransferOnInsertTransfer_r13(tokenId,time);
  ...
  function updateOwnerOfOnInsertLatestTransfer_r0(uint tokenId,address p) private    {
      if(p!=address(0)) {
        ...
        updateTokenNoOwnerOnInsertOwnerOf_r8(tokenId,p);
  ...
  function updateLatestTransferOnInsertTransfer_r13(uint tokenId,uint s) private    {
