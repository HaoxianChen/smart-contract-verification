...
//   }
  ...
  function transferOwnership(address newOwner)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause {
    super.transferOwnership(newOwner);
    transaction = Tx.TransferOwnership;
  }

  function pause()  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause {
    super.pause();
    transaction = Tx.Pause;
  }

  function transfer(address to, uint256 value)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause returns (bool) {
    bool ret = super.transfer(to, value);
    if (ret) transaction = Tx.Transfer;
    return ret;
  }

  function transferFrom(address from, address to, uint256 value)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause returns (bool) {
    bool ret = super.transferFrom(from, to, value);
    if (ret) transaction = Tx.TransferFrom;
    return ret;
  }

  function approve(address spender, uint256 value)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause returns (bool) {
    bool ret = super.approve(spender, value);
    if (ret) transaction = Tx.Approve;
    return ret;
  }

  function increaseApproval(address _spender, uint _addedValue)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause returns (bool) {
    bool ret = super.increaseApproval(_spender, _addedValue);
    if (ret) transaction = Tx.IncreaseApproval;
    return ret;
  }

  function decreaseApproval(address _spender, uint _subtractedValue)  public checkTransferBeforeUnpause checkTransferAfterEndSale checkEvacuateAfterUpgrade checkDeadAfterPause returns (bool) {
