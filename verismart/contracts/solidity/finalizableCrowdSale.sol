...
contract Crowdsale is MintableToken {
  ...
  constructor(uint256 _startBlock, uint256 _endBlock, uint256 _rate) public {
  ...
  //  this method to have crowdsale of a specific mintable token.
  ...
  //  to create custom fund forwarding mechanisms
...
contract CappedCrowdsale is Crowdsale {
  ...
  constructor(uint256 _startBlock, uint256 _endBlock, uint256 _rate, uint256 _cap) 
 	Crowdsale(_startBlock, _endBlock, _rate) public {
    require(_cap > 0);
    cap = _cap;
  }
...
contract FinalizableCrowdsale is Crowdsale {
  ...
  //  this with custom logic if needed
  // function finalization() internal {
  //   token.finishMinting();
  // }
  
  function mint(address _to, uint256 _amount)  public checkBuyAfterFinalization checkEndAfterFinalization returns (bool) {
      bool ret = super.mint(_to,_amount);
      if (ret) {transaction=Tx.Mint;}
	return ret;
  }

  function transfer(address _to, uint256 _amount) public checkBuyAfterFinalization checkEndAfterFinalization returns (bool) {
      bool ret = super.transfer(_to,_amount);
      if (ret) {transaction=Tx.Transfer;}
      return ret;
  }
  function buyTokens(address beneficiary)  public checkBuyAfterFinalization checkEndAfterFinalization payable {
 	super.buyTokens(beneficiary);
        transaction=Tx.BuyTokens;
  }

  function transferFrom(address from, address to, uint256 value)  public checkBuyAfterFinalization checkEndAfterFinalization returns (bool) {
    bool ret = super.transferFrom(from ,to, value);
    if (ret) transaction = Tx.TransferFrom;
    return ret;
  }

  function approve(address spender, uint256 value)  public checkBuyAfterFinalization checkEndAfterFinalization returns (bool) {
    bool ret = super.approve(spender, value);
    if (ret) transaction = Tx.Approve;
    return ret;
  }

  function transferOwnership(address newOwner)  public checkBuyAfterFinalization checkEndAfterFinalization {
    super.transferOwnership(newOwner);
    transaction = Tx.TransferOwnership;
  }

  function finishMinting()  public checkBuyAfterFinalization checkEndAfterFinalization returns (bool) {
