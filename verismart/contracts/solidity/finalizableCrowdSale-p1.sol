...
contract FinalizableCrowdsale is Crowdsale {
  ...
  function finalize() public onlyOwner checkBuyAfterFinalization  {
    require(!isFinalized);
    require(hasEnded());
  ...
  function mint(address _to, uint256 _amount)  public checkBuyAfterFinalization  returns (bool) {
      bool ret = super.mint(_to,_amount);
      if (ret) {transaction=Tx.Mint;}
	return ret;
  }

  function transfer(address _to, uint256 _amount) public checkBuyAfterFinalization  returns (bool) {
      bool ret = super.transfer(_to,_amount);
      if (ret) {transaction=Tx.Transfer;}
      return ret;
  }
  function buyTokens(address beneficiary)  public checkBuyAfterFinalization  payable {
 	super.buyTokens(beneficiary);
        transaction=Tx.BuyTokens;
  }

  function transferFrom(address from, address to, uint256 value)  public checkBuyAfterFinalization  returns (bool) {
    bool ret = super.transferFrom(from ,to, value);
    if (ret) transaction = Tx.TransferFrom;
    return ret;
  }

  function approve(address spender, uint256 value)  public checkBuyAfterFinalization  returns (bool) {
    bool ret = super.approve(spender, value);
    if (ret) transaction = Tx.Approve;
    return ret;
  }

  function transferOwnership(address newOwner)  public checkBuyAfterFinalization  {
    super.transferOwnership(newOwner);
    transaction = Tx.TransferOwnership;
  }

  function finishMinting()  public checkBuyAfterFinalization  returns (bool) {
