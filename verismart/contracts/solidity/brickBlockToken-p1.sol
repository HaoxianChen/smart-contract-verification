...
contract BrickblockToken is PausableToken {
  ...
  function changeBonusDistributionAddress(address _newAddress)
    public
    onlyOwner checkTransferBeforeUnpause   
    returns (bool)
  {
  ...
  function distributeTokens(address _contributor, uint256 _value)
    public
    onlyOwner
    supplyAvailable(_value) checkTransferBeforeUnpause   
    returns (bool)
  {
  ...
  function evacuate(address _user)
    public
    only(successorAddress) checkTransferBeforeUnpause   
    returns (bool)
  {
    require(dead);
...
//   }
  ...
  function transferOwnership(address newOwner)  public checkTransferBeforeUnpause    {
    super.transferOwnership(newOwner);
    transaction = Tx.TransferOwnership;
  }

  function pause()  public checkTransferBeforeUnpause    {
    super.pause();
    transaction = Tx.Pause;
  }

  function transfer(address to, uint256 value)  public checkTransferBeforeUnpause    returns (bool) {
    bool ret = super.transfer(to, value);
    if (ret) transaction = Tx.Transfer;
    return ret;
  }

  function transferFrom(address from, address to, uint256 value)  public checkTransferBeforeUnpause    returns (bool) {
    bool ret = super.transferFrom(from, to, value);
    if (ret) transaction = Tx.TransferFrom;
    return ret;
  }

  function approve(address spender, uint256 value)  public checkTransferBeforeUnpause    returns (bool) {
    bool ret = super.approve(spender, value);
    if (ret) transaction = Tx.Approve;
    return ret;
  }

  function increaseApproval(address _spender, uint _addedValue)  public checkTransferBeforeUnpause    returns (bool) {
    bool ret = super.increaseApproval(_spender, _addedValue);
    if (ret) transaction = Tx.IncreaseApproval;
    return ret;
  }

  function decreaseApproval(address _spender, uint _subtractedValue)  public checkTransferBeforeUnpause    returns (bool) {
