...
contract Escrow {
  ...
  function deposit(address p) onlyOwner public check   payable {
  ...
  function claimRefund(address payable p) public check   {
  ...
  function close() onlyOwner public check   {
    state = State.SUCCESS;
    transaction = Tx.Close;
  }
  function refund() onlyOwner public check   {
    state = State.REFUND;
